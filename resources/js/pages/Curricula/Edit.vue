...
// Form state - Initialize with existing curriculum data
const form = useForm({
  program_id: props.curriculum.program_id,
  school_year: props.curriculum.school_year,
  year_level: props.curriculum.year_level,
  semester: props.curriculum.semester,
  tuition_per_unit: props.curriculum.tuition_per_unit,
  lab_fee: props.curriculum.lab_fee,
  registration_fee: props.curriculum.registration_fee,
  misc_fee: props.curriculum.misc_fee,
  term_count: props.curriculum.term_count,
  courses: [...props.selectedCourses],
  notes: props.curriculum.notes || '',
  is_active: props.curriculum.is_active,
})

// Available courses based on selected program
const availableCourses = ref<Course[]>([...props.courses])
const loadingCourses = ref(false)

// Watch program selection to load courses
watch(() => form.program_id, async (newProgramId) => {
  if (newProgramId && newProgramId !== props.curriculum.program_id) {
    loadingCourses.value = true
    form.courses = [] // Clear selected courses when changing program
    try {
...
const totalUnits = computed(() => {
  return selectedCoursesDetails.value.reduce((sum, course) => sum + course.total_units, 0)
})

const totalLabCourses = computed(() => {
  return selectedCoursesDetails.value.filter(course => course.has_lab).length
})
...
<template>
  <Head :title="`Edit Curriculum - ${curriculum.program.code}`" />
  ...
  <AppLayout>
    <div class="space-y-6 p-6 max-w-7xl mx-auto">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold">Edit Curriculum</h1>
          <p class="text-gray-600 mt-2">
            Update curriculum details, courses, and fee structure
          </p>
        </div>
      ...
      <!-- Warning if curriculum has enrolled students -->
      <div 
      ...
      <form @submit.prevent="submitForm" class="space-y-6">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <!-- Left Column: Basic Info & Courses -->
          <div class="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Update program and term details
                </CardDescription>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Update tuition, lab, and miscellaneous fees
                </CardDescription>
              </CardHeader>
              <CardContent class="space-y-4">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div class="flex items-start gap-2">
                    <Info class="h-4 w-4 text-blue-600 mt-0.5" />
                    <div class="flex-1">
                      <p class="text-sm font-medium text-blue-900">
                        Miscellaneous Fee Breakdown ({{ formatCurrency(calculatedMiscFee) }})
                      </p>
                      <div class="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-xs text-blue-700">
            ...
            <Card>
              <CardHeader>
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Update courses included in this curriculum
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <div v-if="loadingCourses" class="text-center py-8 text-gray-500">
                  Loading courses...
                </div>
            ...
            <Card class="sticky top-6">
              ...
              <CardContent class="space-y-4">
                ...
                <div class="flex justify-between text-sm">
                  ...
                  <span class="font-medium">{{ selectedCoursesDetails.length }}</span>
                </div>
                ...
                <div class="pt-4 space-y-2">
                  <Button
                    type="submit"
                    class="w-full"
                    :disabled="form.processing || !selectedCoursesDetails.length"
                  >
                    <Save class="w-4 h-4 mr-2" />
                    {{ form.processing ? 'Saving...' : 'Save Changes' }}
                  </Button>
                  
                  <Link :href="route('curricula.show', curriculum.id)" class="block">
